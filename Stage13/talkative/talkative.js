...
.directive('tktvRemoveItem', function() {
  return {
    restrict: 'E',
    templateUrl: 'talkative/templates/tktv-remove-item.html',
    controller: ['$scope', '$http', function($scope, $http) {
      $scope.idToDelete = '';
      $scope.removeItem = function() {
        $http({method: 'DELETE', url: 'http://localhost:6969/api/todo/items/' + $scope.idToDelete})
          .success(function(results) {
            $scope.idToDelete = '';
            toastr.success('Item successfully blown away, it won\'t be troubling you no more!');
          })
          .error(function (e) {
            toastr.error('Either that wasn\'t a valid Id or the server is having a really bad day.');
          });
